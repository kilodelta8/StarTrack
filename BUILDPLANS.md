...
| *$12\text{V } 5\text{A}$ Power Supply* | 1         | Dedicated for the motors and drivers                         |
| *Limit Switches*                     | 3         | $2$ for Azimuth Home/Limits, $1$ for Elevation Home          |
| *$5\text{V}$ Regulator*               | 1         | To step down $12\text{V}$ to clean $5\text{V}$ for the ESP32/Uno |
...
*   **Azimuth Bearing:** Install a large thrust or radial bearing into the center of the base frame. This bearing carries the payload weight ($2.1\text{kg}$ max) and is the pivot point for the Azimuth shaft.
...
*   **First Reduction Stage:** Mount a small drive pulley ($20\text{T}$) onto the motor shaft and a medium driven pulley ($100\text{T}$) onto a small intermediate shaft, achieving a $5:1$ reduction.
*   **Second Reduction Stage:** Mount a small drive pulley (e.g., $20\text{T}$) onto the intermediate shaft, which drives a very large gear/pulley (e.g., $400\text{T}$) fixed to the main vertical Azimuth shaft. This yields $5:1 \times 20:1 \approx 100:1$ total ratio.
...
Before connecting the motors, set the current limit ($I_{MAX}$) on the *DRV8825* drivers using the trim pot.

```
$$\text{V_{REF}} = \text{I_{MAX}} \times 0.9$$
```

*   Consult your *NEMA 17* motor's datasheet for its rated phase current ($I_{MAX}$). (Typical *NEMA 17* motors are $1.5\text{A}$ or $1.7\text{A}$).
*   Measure the voltage on the `V_REF` pin while turning the trim pot.
*   Set `V_REF` to the calculated value (e.g., for a $1.5\text{A}$ motor, set `V_REF` to $1.35\text{V}$). This protects the motor and driver.

### 4.2 Power Distribution

*   **$12\text{V}$ Rail:** Connect the $12\text{V}$ supply directly to the *DRV8825* motor power inputs (`VMOT` and `GND`).
*   **$5\text{V}$ Rail:** Use the separate $5\text{V}$ regulator to create a clean power source. Connect this $5\text{V}$ rail to the *Arduino Uno*'s `5V` pin and the *ESP32*'s `5V` input pin (`VBUS` or similar).
...
*   **Initial Power Test:** Apply $12\text{V}$ power and verify the $5\text{V}$ rail is correct. **Do not** connect the Uno/ESP32 USB during this test unless necessary, relying solely on the $5\text{V}$ regulator.
